...
func sqaureCuber(sqInChan, sqOutChan, cuInChan, cuOutChan, exitChan chan int) {
	var squareX int
	var cubeX int
	for {
		select {
...
func main() {
	...
	go sqaureCuber(sqInChan, sqOutChan, cuInChan, cuOutChan, exitChan)
	for i := 0; i < 10; i++ {
		sqInChan <- i
		cuInChan <- i
	}
